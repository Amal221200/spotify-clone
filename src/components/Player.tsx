...
const Player = () => {
  const { currentSong, play, onMuted, muted, onPlay, onPlaying, onFinished } = useContext(PlayerContext) as TPlayerContext;
  ...
  return (
    <div className='mx-auto flex h-full max-w-80 items-center justify-center sm:w-full'>
      <audio onPlaying={() => onPlaying(audioRef, progressBarRef, progressBarPointerRef)} ref={audioRef as LegacyRef<HTMLAudioElement>} src={currentSong.url} autoPlay={play} onEnded={() => { onFinished(progressBarRef, progressBarPointerRef)}} />
      <div className="flex flex-col gap-y-4 sm:w-[350px] md:w-[350px] xl:w-[400px]">
        ...
        <div>
          <div className="group relative mb-4 h-[2px] w-full cursor-pointer rounded-lg bg-gray-50/50">
            <div ref={progressBarRef as LegacyRef<HTMLDivElement>} className="absolute left-0 top-0 h-[2px] rounded-lg bg-gray-50" style={{ width: '0' }} />
            <div ref={progressBarPointerRef as LegacyRef<HTMLDivElement>} className="absolute -top-[3px] h-2 w-2 translate-y-0 scale-0 transform rounded-full bg-gray-50 transition-all group-hover:scale-100" style={{ left: '0' }} />
          </div>
